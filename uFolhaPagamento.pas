...
begin
  ...
  if cdsFolhaPagamento.FindKey([wCodigoFuncionario]) then
     begin
       cdsFolhaPagamento.Edit;

       cdsFolhaPagamentobdNOME.AsString := wNomeFuncionario;
       cdsFolhaPagamentobdCARGO.AsString := wCargoFuncionario;
...
begin
  ...
  edCodigoFuncionario.Enabled   := True;
  edNomeFuncionario.Enabled     := True;
  cbCargo.Enabled               := True;
  edEndereco.Enabled            := True;
  edTelefone.Enabled            := True;
  btSalvarFuncionario.Visible   := True;
end;
...
begin
  ...
  edNomeFuncionario.Enabled     := False;
  cbCargo.Enabled               := False;
  edEndereco.Enabled            := False;
  edTelefone.Enabled            := False;
  btSalvarFuncionario.Visible   := False;
end;
...
begin
  wCodigoFolha := StrToIntDef(edCodigo.Text,0);
