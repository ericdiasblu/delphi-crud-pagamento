...
type
  ...
  private
    ...
    // Controle de busca
    wBuscouPorEnter: Boolean;
    wFolhaEncontrada: Boolean;
    ...
    function fExisteFolhaFuncionario: Boolean;
...
begin
  if wBuscouPorEnter then
     begin
       wBuscouPorEnter := False;
       Exit;
     end;
...
begin
  ...
  if not cdsFolhaPagamento.FindKey([edCodigo.Text]) then
     begin
       if fExisteFolhaFuncionarioMesAno then
          begin
            ShowMessage('Funcionário já possui folha neste mês e ano');
            Exit;
          end;
...
begin
  cdsFolhaPagamento.IndexFieldNames := 'bdCODIGO';
  wFolhaEncontrada := True;

  if cdsFolhaPagamento.FindKey([edCodigo.Text]) then
     begin
       cdsFuncionario.IndexFieldNames := 'bdCODIGOFUNCIONARIO';

       if not cdsFuncionario.FindKey([cdsFolhaPagamentobdCODIGOFUNCIONARIO.AsInteger]) then
          begin
       ...
       cbNome.ItemIndex               := cbNome.Items.IndexOf(cdsFuncionariobdNOMEFUNCIONARIO.AsString);
       edCargo.Text                   := cdsFuncionariobdCARGO.AsString;
...
begin
  ...
  if Pos(' ', wTelefoneFuncionario) > 0 then
...
begin
  ...
  if wTotalProventos <= 1621 then
     ...
     begin
       ...
       wSegundaFaixa  := fCalcularSegundaFaixa;
       wTerceiraFaixa := fCalcularTerceiraFaixa;
       wQuartaFaixa   := (8475.55 - 4354.27) * 0.14;
     end;
...
begin
  ...
  if cdsFuncionario.FindKey([edCodigoFuncionario.Text]) then
     begin
       if fExisteFolhaFuncionario then
          begin
            ShowMessage('Não é possível excluir um funcionário que possui folhas cadastradas.');
            Exit;
          end;

       if MessageDlg('Deseja excluir este funcionário?', mtConfirmation, [mbYes, mbNo], 0) = mrYes then
          begin
            cbNome.Items.Delete(cbNome.Items.IndexOf(cdsFuncionariobdNOMEFUNCIONARIO.AsString));
...
function TfrFolhaPagamento.fExisteFolhaFuncionarioMesAno: Boolean;
begin
  Result := False;
  ...
  while not cdsFolhaPagamento.Eof do
    begin
      if (cdsFolhaPagamentobdCODIGOFUNCIONARIO.AsInteger = cdsFuncionariobdCODIGOFUNCIONARIO.AsInteger) and (cdsFolhaPagamentobdMES.AsString = cbMes.Text) and (cdsFolhaPagamentobdANO.AsString = edAno.Text) and (cdsFolhaPagamentobdCODIGO.AsInteger <> StrToIntDef(edCodigo.Text, 0)) then
         begin
           Result := True;
           Exit;
         end;
...
procedure TfrFolhaPagamento.edCodigoKeyPress(Sender: TObject;
  var Key: Char);
begin
  if Key = #13 then
     begin
...
begin
  wBaseCalculo := wTotalProventos - wInss;

  if wBaseCalculo <= 2428.80 then
     Result := 0

  else if wBaseCalculo <= 2826.65 then
     Result := (wBaseCalculo * 0.075) - 182.16

  else if wBaseCalculo <= 3751.05 then
     Result := (wBaseCalculo * 0.15) - 394.16

  else if wBaseCalculo <= 4664.68 then
     Result := (wBaseCalculo * 0.225) - 675.49
     
  else
     Result := (wBaseCalculo * 0.275) - 908.73;

  if Result < 0 then
     Result := 0;
end;

function TfrFolhaPagamento.fExisteFolhaFuncionario: Boolean;
begin
  Result := False;
